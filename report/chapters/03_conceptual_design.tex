...
根据当前数据库结构反向抽取，系统实体可以分为组织结构、用户身份、教学安排、选课成绩和审计记录五组。
...
在数据库原理1系统基础上的规范化优化中，上课时间不作为自由文本保存在 \code{course_offering} 中，而是由 \code{course_schedule(schedule_id, offering_id, weekday, start_section, end_section)} 表示。一个教学班可对应多个时间片，便于后续进行时间冲突检查。
...
图 \ref{fig:er-conceptual} 采用实体--联系图表达方式：矩形表示实体，菱形表示联系，连线上的 \code{1}、\code{N}、\code{0..1} 表示基数约束。为保证报告中的可读性，图中只保留每个实体的核心属性；完整字段、主键、外键和唯一约束在第 4 章逻辑关系模型中进一步展开。
