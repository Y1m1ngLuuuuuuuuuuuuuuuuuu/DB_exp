...
\code{db/connection.py} 中的 \code{DBSession} 是项目的事务封装。它进入上下文时建立连接，正常退出时提交事务，发生异常时回滚事务。本轮改造后，学生选课、退课和成绩更新都使用显式事务路径。

学生选课由 \code{services/selection_service.py} 调用数据库函数 \code{select_course_tx(p_student_id, p_offering_id)}。openGauss 存储函数本身不显式开始或提交事务，而是在调用方事务中执行带锁检查和写入。因此事务边界由 \code{DBSession} 管理，数据库函数和触发器在同一事务上下文内工作。

\section{选课事务流程}

当前选课事务流程见表 \ref{tab:select-transaction-flow}。其核心原则是：容量检查、时间冲突检查、先修课程检查和写入 \code{enrollment} 必须处于同一事务边界内。
...
7 & 检查时间冲突 & \code{course_schedule}、\code{enrollment} & 同一学生同学期课程时间不重叠 & 同一事务内查询 \\
8 & 检查先修课程 & \code{course_prerequisite}、\code{enrollment} & 未通过先修课不能选后续课 & 同一事务内查询 \\
9 & 由唯一约束兜底 & \code{enrollment} & 防止重复记录落库 & \code{UNIQUE(student_id, offering_id)} \\
\end{longtable}
}
...
由于同一教学班的容量检查被串行化，两个事务不会同时基于同一个旧容量状态插入记录，因此可以防止“最后一个名额被多人同时抢到”的超选问题。系统没有恢复 \code{selected_count} 字段，而是通过锁加聚合统计实现一致性。
...
成绩更新也已经改为事务化。服务层在同一 \code{DBSession} 中锁定目标 \code{enrollment} 行，检查教师或管理员权限，然后设置审计上下文：
...
当前系统没有显式调整全局事务隔离级别，而是针对高风险资源使用行级锁。对选课容量而言，锁定 \code{course_offering} 行已经能把同一教学班的容量检查串行化；对重复选课而言，业务唯一约束可以在较低隔离级别下兜底；对成绩更新而言，服务层锁定目标 \code{enrollment} 行后再更新成绩，避免同一成绩记录并发覆盖。

\section{当前实现的不足与改进}

表 \ref{tab:concurrency-risk-solution} 汇总了当前系统面向典型并发场景的处理方式。
...
当前已处理最核心的抢课容量风险，但仍有改进空间：同一学生同一学期选择同一课程不同教学班的限制尚未固化，需要跨表触发器或额外业务键设计；此外，还应继续编写多连接并发压测，模拟多个学生同时抢同一教学班最后名额，验证锁等待和异常回滚行为。

\section{本章小结}

本章从事务和锁角度说明了当前系统如何处理选课一致性。系统没有依赖页面端“先查容量再插入”的脆弱流程，而是在 \code{DBSession} 事务中调用 \code{select_course_tx()}，由行级锁、触发器和唯一约束共同保证选课写入的一致性。成绩更新同样通过事务上下文和触发器保证成绩与审计日志原子一致。
