...
BEGIN
    ...
    SELECT COUNT(*),
           STRING_AGG(cp.prereq_course_id || ' ' || c_prereq.course_name, ', ' ORDER BY cp.prereq_course_id)
      INTO v_missing_prereq_count, v_missing_prereqs
    FROM course_prerequisite cp
    JOIN course c_prereq
      ON c_prereq.course_id = cp.prereq_course_id
    ...
    IF v_missing_prereq_count > 0 THEN
        RAISE EXCEPTION 'Student % has not passed all prerequisites for course %. Missing prerequisites: %',
            NEW.student_id, v_course_id, v_missing_prereqs;
    END IF;
