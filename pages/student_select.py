...
def render() -> None:
    ...
    has_seats_only = f4.checkbox("仅看有余量", value=True)
    ...
    with tab_avail:
        ...
        render_section_title("可选课程", f"共 {len(available)} 个教学班符合条件。")
        if not available:
        ...
        for row in available:
            ...
            b2.caption("数据库会检查容量、时间冲突和先修课程。")

    with tab_enrolled:
        enrolled = list_enrolled_offerings(student_id, semester_id)
        render_section_title("已选课程", f"共 {len(enrolled)} 条选课记录。")
        if not enrolled:
            render_empty_state("本学期尚未选择课程", "选课成功后会显示在这里。")
        for row in enrolled:
